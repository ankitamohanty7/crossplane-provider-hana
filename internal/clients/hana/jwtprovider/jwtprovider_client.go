...
// provider. Returns nil when the provider does not exist. Straight-line SQL +
// column-by-column scan; splitting adds indirection without reducing branches.
//
//nolint:gocyclo
...
// Update reconciles drift between desired and observed state. One branch per
// JWT provider attribute; the shape mirrors the DDL surface and is intentionally flat.
//
//nolint:gocyclo
func (c Client) Update(ctx context.Context, parameters *v1alpha1.JWTProviderParameters, observation *v1alpha1.JWTProviderObservation) error {
	...
	observedAppUser := ""
	if observation.ApplicationUserClaim != nil {
		observedAppUser = *observation.ApplicationUserClaim
	}
	if desiredAppUser != observedAppUser {
		if desiredAppUser == "" {
			// HANA syntax to drop the binding: UNSET CLAIM '<claim>'.
			if observedAppUser != "" {
				query := fmt.Sprintf("ALTER JWT PROVIDER %s UNSET CLAIM '%s'", parameters.Name, utils.EscapeSingleQuotes(observedAppUser))
				if _, err := c.ExecContext(ctx, query); err != nil {
